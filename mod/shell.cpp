...
bool parse_command(char *command_out[], char *command);
...
void* wait_thread(void* arg) {
  ...
  while (me->alive) {
    sleep(me->interval);
    if (parse_command(command_execv, me->command->getstr())) {
      execute_command(command_execv, me->destination->getstr(), me->server); 
    }
  }
}
